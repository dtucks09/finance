...
def trends(ticker):
    ...
    df['20d_stdev'] = df['Close'].rolling(window=20).std()

    df['UpperBand'] = df['20d_sma'] + (df['20d_stdev'] * 2)
    df['LowerBand'] = df['20d_sma'] - (df['20d_stdev'] * 2)

    risk_range = list(
    df['Close'].tail(1).values.tolist() +
    df['5d_sma'].tail(1).values.tolist() +
    df['10d_sma'].tail(1).values.tolist() +
    df['20d_sma'].tail(1).values.tolist() +
    ...
    df['LowerBand'].tail(1).values.tolist() +
    df['UpperBand'].tail(1).values.tolist() +
